...
class ProofBuilder:
    ...
    async def build_proof(
        ...
        # 5. Get header chain from checkpoint+1 to target
        ...
        # 6. Get target block and build Merkle proof
        ...
        # Verify against header's Merkle root
    ...
    async def verify_proof_locally(self, result: ProofBuildResult) -> bool:
        ...
        target_header = BlockHeader.from_bytes(result.proof.headers[-1])
        txid = sha256d(result.proof.raw_tx)

        if not verify_merkle_proof(
            txid,
            target_header.merkle_root,
