...
class TestModels:
    ...
    def test_set_checkpoint_request_validation(self):
        ...
        req = SetCheckpointRequest(height=2500000)
        assert req.height == 2500000


class TestAuth:
    """Tests for tokenless endpoint access."""
    ...
    def test_protected_endpoint_no_token_no_auth(self, client):
        """Protected endpoints should not enforce token headers."""
        response = client.post(self.PROTECTED_ENDPOINT, json=self.VALID_REQUEST_BODY)
        assert response.status_code != 401

    def test_extra_header_does_not_change_access(self):
        """Extra headers should not change access behavior."""
        with TestClient(app) as test_client:
            response = test_client.post(
                self.PROTECTED_ENDPOINT,
                json=self.VALID_REQUEST_BODY,
                headers={"X-Debug": "ignored"},
            )
            assert response.status_code != 401
