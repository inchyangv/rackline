...
class SetCheckpointRequest(BaseModel):
    """Request to build a Bitcoin checkpoint payload for wallet submission."""
...
class SetCheckpointResponse(BaseModel):
    """Response containing checkpoint payload data."""

    success: bool = Field(..., description="Whether payload was built successfully")
    ...
    dry_run: bool = Field(True, description="Always true: API never sends tx")
    error: Optional[str] = Field(None, description="Error message if failed")
...
class ClaimCompleteRequest(BaseModel):
    """Request to verify a borrower claim and derive mapping hashes."""
...
class ClaimCompleteResponse(BaseModel):
    """Response from verifying a borrower claim."""
    ...
    dry_run: bool = Field(True, description="Always true: API does not send tx")
    error: Optional[str] = Field(None, description="Error message if failed")
