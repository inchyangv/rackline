...
ProofEnvelope ─▶ VerifiedSourceEvent ─▶ BusinessEvidence ─▶ EligibleUnpaidReceivable ─▶ (decision, draw)
 untrusted        native fact              our interpretation      policy + issuer state         GPU-081
```
...
`nativeStatus=NATIVE_ACCEPTED` is a statement about a source log only (R2-D05).
...
## 3. VerifiedSourceEvent — the proof-bound locator and ID contract

| Field | Bound by | Source |
| --- | --- | --- |
| `envId` | manifest (`manifestHash`) of the verifier deployment | GPU-075 manifest |
...
| `logOrdinal` | index inside `receiptLogs[]` of the decoded receipt (0-based) — **not** RPC block-global `logIndex` | verified bytes |
...
- `sourceEventId = keccak256(abi.encode(keccak256(envId), uint64 chainKey, uint64 height, uint64 txIndex,
  uint32 logOrdinal))` — the **consumption key**; one consumption per id per environment. Vectors:
  `canonical-id-vectors-v1.json` (Python and TypeScript agree; Solidity in GPU-029/031).
- `proofQueryKey = (envId, chainKey, txHash)` — the **cache key** of the proof service. Never a consumption
...
- `economicEventId` (domain-model §2) is derived from provider/account/eventType/provider ref, **never**
  ...
  new verifier or manifest re-consumes nothing (GPU-082 regression).
- Same `chainKey` means different chains per environment (cc3-testnet: 1 = Sepolia; cc3-mainnet: 1 =
  ...
  every record and production readers reject anything else (GPU-011/015).
...
The internal proposal for Rackline-controlled source contracts (TEST_ONLY MockDePIN and any escrow we
deploy, GPU-077) is `source-events-v1.abi.json`. Real partner ABIs (Aethir/GPU.net) are **external and
unconfirmed**; they are recorded per partner when GPU-004/005 deliver them, and mapped through the same
...
base needs natively provable **obligation/assignment** events **or** an approved alternative; without one,
admission for that partner is **OPEN (R2-O03)** and no production eligibility exists (completion rule).
...
## 6. Freshness and omission defense (R2-D06)
...
- **Bounded lag / buffer**: the pilot allows no lag (`0`); any bounded-lag or buffer policy is a
  GPU-007/010 approval and is recorded in `policy_versions`.
- **Reservation**: while a draw is being executed, the receivables it relies on are `reservedDraws`
  on the facility; a correction arriving in between revokes the decision before execution (GPU-081).
...
| Source dispute after a draw/repay (issuer corrects or cancels) | receivable → `DISPUTED`/corrected revision; decision revoked; new draws blocked; **existing destination debt, transfers, and received repayments are never erased**; reconciliation and agreement procedure (GPU-013 §4, §6) |
...
| Control PoC + partner recognition (GPU-009) | `controlGrade` | receivables |
...
| ID | Question | Blocks |
| --- | --- | --- |
| EC-O01 | Which partner events are natively provable obligation/assignment events (vs payouts only)? | production admission (R2-O02/O03) |
...
| EC-O03 | Partner event ABIs, decimals, issuer roles, escrow semantics | GPU-004/005/008/077 |

## 10. Vectors handed to GPU-014/029
...
amounts (Python ↔ TypeScript verified; Solidity in GPU-029).
