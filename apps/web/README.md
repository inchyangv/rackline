...
- **Dashboard** — Read on-chain state (credit limit, debt, payout history)
- **Operations** — Build checkpoint payload via API, then submit `setCheckpoint` with wallet
- **Proof** — Build proof via API, then submit `submitPayout` with wallet
- **Admin** — Wallet-based borrower mapping (`registerBorrower`, `setBorrowerPubkeyHash`, `setVerifier`)
- **Settings** — RPC and contract configuration
