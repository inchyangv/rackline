...
const test = base.extend<{ demoGuard: void }>({
  demoGuard: [
    async ({ context, page }, use) => {
      ...
      await page.addInitScript(() => {
        ...
        Reflect.set(window, "ethereum", {
          ...
          request: async ({ method }: { method: string }) => {
            const record = Reflect.get(window, "recordDemoWalletRequest") as (
              value: string,
            ) => Promise<void>;
            await record(method);
      ...
      expect(
        walletRequests,
        "Demo interactions must never invoke an injected wallet",
      ).toEqual([]);
      expect(
        externalRequests,
        "The demo must not request an external API or RPC",
      ).toEqual([]);
...
async function readState(page: Page): Promise<DemoState> {
  return page.evaluate((key) => {
    ...
    if (!persisted)
      throw new Error(
        "Expected demo state to be persisted after an interaction",
      );
...
async function transact(
  ...
  amount: string,
  result: "complete" | "queued" = "complete",
) {
  await page
    .getByRole("main")
  ...
  const operation =
    action === "Repay"
      ...
      : action === "Withdraw"
        ...
        : action.toLowerCase();
  await dialog
    .getByRole("button", { name: `Review ${operation}`, exact: true })
    .click();
  await dialog
    .getByRole("button", { name: `Confirm demo ${operation}`, exact: true })
    .click();
  const completed =
    action === "Repay"
      ? "Repayment"
...
test("renders the demo and navigates every screen without horizontal page overflow", async ({
  page,
}, testInfo) => {
  await page.goto("/");
  await expect(
    page.getByRole("button", { name: "Rackline overview", exact: true }),
  ).toBeVisible();
  await expect(
  ...
  for (const screen of [
    ...
    expect(
      overflow,
      `${screen} must fit within the ${testInfo.project.name} viewport`,
    ).toBeLessThanOrEqual(1);
  }
});

test("supply, withdraw, borrow and repay update demo balances and survive a reload", async ({
  page,
}) => {
  await page.goto("/");
  await page
  ...
  expect(repaid.debt + repaid.interest).toBeCloseTo(
    borrowed.debt + borrowed.interest - 50,
    2,
  );
...
test("invalid amounts cannot reach confirmation or change balances", async ({
  page,
}) => {
  await page.goto("/");
  await page
  ...
  for (const amount of ["-1", "900000000"]) {
    ...
    await expect(
      dialog.getByRole("button", { name: "Review supply", exact: true }),
    ).toBeDisabled();
...
test("connecting a sample provider requires consent and does not approve credit", async ({
  page,
}) => {
  await page.goto("/");
  await page
  ...
  await dialog
    .getByRole("checkbox", { name: /does not approve a loan/ })
    .check();
  await expect(
    dialog.getByRole("button", { name: "Continue", exact: true }),
  ).toBeDisabled();
  await dialog
    .getByRole("checkbox", { name: /create a sample connection/ })
    .check();
  await dialog.getByRole("button", { name: "Continue", exact: true }).click();
  await expect(
    dialog.getByText("Pending review", { exact: true }),
  ).toBeVisible();
  await dialog
    .getByRole("button", { name: "Add sample connection", exact: true })
    .click();
  ...
  await dialog
    ...
    .click();
  await expect(
    page.getByRole("main").getByText("QA demo cluster", { exact: true }),
  ).toBeVisible();
  expect(await readState(page)).toMatchObject(initial);
});

test("proof and control interruptions block borrowing while direct repayment remains available", async ({
  page,
}) => {
  await page.goto("/");
  await page
  ...
  for (const scenario of ["proof-pending", "control-expired"]) {
    ...
    await expect(
      main.getByRole("button", { name: "Borrow", exact: true }),
    ).toBeDisabled();
    await expect(
      main.getByRole("button", { name: "Repay", exact: true }),
    ).toBeEnabled();
    const interrupted = await readState(page);
    expect(interrupted.debt + interrupted.interest).toBeCloseTo(
      before.debt + before.interest,
      2,
    );
    await transact(page, "Repay", "1");
    const repaid = await readState(page);
    expect(repaid.debt + repaid.interest).toBeCloseTo(
      before.debt + before.interest - 1,
      2,
    );
  }
...
test("a queued withdrawal requires liquidity before settlement and credits the wallet only once on claim", async ({
  page,
}) => {
  await page.goto("/");
  await page
  ...
  const main = page.getByRole("main");
  await main
    .getByRole("button", { name: "Try limited liquidity", exact: true })
    .click();
  ...
  expect(stressed.debt + stressed.interest).toBe(
    initial.debt + initial.interest,
  );

  await transact(page, "Withdraw", "6000", "queued");
  const queued = await readState(page);
  expect(queued.queue).toEqual([
    { id: expect.any(String), amount: 6000, status: "pending" },
  ]);
  ...
  await main
    .getByRole("button", { name: "Simulate settlement", exact: true })
    .click();
  expect(await readState(page)).toMatchObject(queued);
  await expect(
    main.getByRole("button", { name: "Claim demo funds", exact: true }),
  ).toHaveCount(0);
  ...
  expect(repaid.debt + repaid.interest).toBeCloseTo(
    initial.debt + initial.interest - 1000,
    2,
  );
  await navigate(page, "Earn");
  await main
  ...
  await main
    .getByRole("button", { name: "Claim demo funds", exact: true })
    .click();
  ...
  await expect(
    main.getByText("Nothing in the queue", { exact: true }),
  ).toBeVisible();
  await expect(
    main.getByRole("button", { name: "Claim demo funds", exact: true }),
  ).toHaveCount(0);
